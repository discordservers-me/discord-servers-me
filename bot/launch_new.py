...
def on_ready():
    ...
    if not settings.DEBUG:
        ...
        presence = f'Prefix: r!'
        yield from bot.change_presence(game=discord.Game(name=presence))
...
@bot.async_event
def on_guild_update(guild_before, guild_after):
...
def bump_premium_servers():
    ...
    while not bot.is_closed():
        ...
        for server in (list(tier1) + list(tier2)):
            ...
            server.save()
            print('Bumped')
        yield from asyncio.sleep(settings.BUMP_DURATION)


@asyncio.coroutine
def check_changed_manager():
    pass
